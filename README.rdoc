...
Run the ruby script when it is changed (works with ruby, perl, python and awk):

   $ filewatcher -r ruby-script.rb
...
Options:
  --interval, -i <f>:   Interval to scan filesystem. Defaults to 0.5 seconds.
           --run, -r:   Run file as a program when file is updated.
